...
class TorrentTableParser(BaseParser):
    ...
    """
    _xpath_table = ".//div[@id='content']/div[@id='main-content']"\
    ...
    def process_page_numbers(self):
        ...
        *Returns*
        ...
        if len(element) != 1:
            raise exceptions.\
                ElementNotFound("Correct element for page numbers not found")
        element = element[0]
    ...
    def run(self):
        ...
        append all parsed row data into a list. That will be returned with the
        other data such as current page and number of pages.
...
class SearchPageParser(TorrentTableParser):
    ...
    def process_row(self, row):
        ...
        if len(columns) != 3:
            raise exceptions.InvalidRow("Row isn't valid.")
...
class TorrentInfoParser(BaseParser):
    ...
    """
    _xpath_details_frame = ".//div[@id='content']/div[@id='main-content']/div"\
                        "/div[@id='detailsouterframe']/div[@id='detailsframe']"
    ...
    def locate_element_with_xpath(self, xpath_list, err_msg):
        """Locate an element, from a list of xpath expressions.

        Raise exceptions.ElementNotFound with err_msg if the xpath
            ...
            for child in dl.iterchildren(tag="dt"):
                ...
                if label == "type":
                    ...
                    if a_link is not None:
                        ...
                        if len(category) > 1:
                            data["category"] =\
                            cat[category[0].strip()][category[1].strip()]
                        else:
                    ...
                    size, unit = dd.text_content().strip().\
                                    replace(u"\xa0", " ").split(" ")[:2]
                    size = float(size)
                    ...
                    date = datetime.datetime.\
                            strptime(dd.text.strip(), "%Y-%m-%d %H:%M:%S %Z")
